...
class BaseProgressWriter(object):
    ...
    def update_with_trainer(self, trainer, with_metric=False):
        '''
        Updates the current loss, the minibatch_size and optionally the metric using the information from the
        ``trainer``.
    ...
    def summarize_progress(self, with_metric=False):
        ...
        Args:
            with_metric (`bool`): if `False` write only the loss summary, otherwise write both the loss and the metric
             summaries.
        '''
        self.epochs += 1
